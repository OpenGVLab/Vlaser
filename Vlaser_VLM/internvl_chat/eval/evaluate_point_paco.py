...
def get_gt_segmentation(segs,hw):
    ...
    for seg in segs:
        ...
        total_mask = np.logical_or(total_mask, mask_np)
...
class PacoLvisDataset(torch.utils.data.Dataset):
            ...
            for line in f:
                ...
                for conv in item['conversations']:
                    if conv['from'] == 'human':
                        ...
                        expr = match.group(1)
                        self.datas.append({
                            'image': os.path.join('/mnt/petrelfs/share_data/zhangtianyi1/paco-lvis', image_path),
                            'expression': expr,
                            'width': w,
                            'height': h,
...
def collate_fn(batches, tokenizer):
    ...
    seg = [_['segmentation'] for _ in batches]
    return pixel_values, texts, images, expressions, hws, seg
...
def evaluate_paco_lvis(args):
    ...
    for index, (pixel_values, questions, images, expressions, hws, seg) in enumerate(tqdm(dataloader)):
    ...
    for result in pred_results:
        ...
        masks = get_gt_segmentation(result['seg'], result['hw'])

        for p in pred_points:
            unnorm_x = int(p[0] / 1000 * result['hw'][1])
            unnorm_y = int(p[1] / 1000 * result['hw'][0])
            if unnorm_x < result['hw'][1] and unnorm_y < result['hw'][0]:
                if masks[unnorm_y,unnorm_x]:
                    correct += 1
    ...
    out_path = os.path.join(args.out_dir, f'paco_eval.json')
    json.dump(pred_results, open(out_path, 'w'), indent=2)
    out_acc_path = os.path.join(args.out_dir, f'paco_eval_acc.json')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-path', type=str, default="/mnt/petrelfs/share_data/zhangtianyi1/paco_internvl_format/paco_lvis_v1_pointing_test_single_conv.jsonl", help='Path to PixMo-Points jsonl file')
