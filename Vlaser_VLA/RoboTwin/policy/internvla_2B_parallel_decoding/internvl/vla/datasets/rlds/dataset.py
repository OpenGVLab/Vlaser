...
from internvl.vla.constants import ACTION_DIM, ACTION_PROPRIO_NORMALIZATION_TYPE, ACTION_TOKEN_BEGIN_IDX, IGNORE_INDEX, NUM_ACTIONS_CHUNK, PROPRIO_DIM, STOP_INDEX
from internvl.vla.datasets.rlds import obs_transforms, traj_transforms
from internvl.vla.datasets.rlds.utils import goal_relabeling, task_augmentation
...
def make_single_dataset(
    ...
    *,
    ...
    Args:
        ...
        shuffle_buffer_size: Buffer size for shuffling data (only used in training).
    """
    dataset, dataset_statistics = make_dataset_from_rlds(
        **dataset_kwargs,
        train=train,
    )
    dataset = apply_trajectory_transforms(dataset, **traj_transform_kwargs, train=train).flatten(num_parallel_calls=tf.data.AUTOTUNE)
    ...
    if train and shuffle_buffer_size > 0:
        dataset = dataset.shuffle(shuffle_buffer_size)
        print(f"Applied shuffle with buffer size: {shuffle_buffer_size}")
