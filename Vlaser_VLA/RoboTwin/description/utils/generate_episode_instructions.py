...
def load_scene_info(task_name: str, setting: str, scene_info_path: str) -> Dict[str, Dict]:
    ...
    file_paths = [os.path.join(parent_directory, f"../../{scene_info_path}/{task_name}/{setting}/scene_info_{num}.json")
                 for num in range(8)]


    # # 多台机器跑同一个task
    # file_paths = []
    # file_paths.extend([os.path.join(parent_directory, f"../../{scene_info_path}_1/{task_name}/{setting}/scene_info_{num}.json")
    #              for num in range(8)])
    #
    # file_paths.extend([os.path.join(parent_directory, f"../../{scene_info_path}_2/{task_name}/{setting}/scene_info_{num}.json")
    #              for num in range(8)])
    # file_paths.extend([os.path.join(parent_directory, f"../../{scene_info_path}_0/{task_name}/{setting}/scene_info_{num}.json")
    #              for num in range(8)])

    data_all = {}
    # file_path = os.path.join(parent_directory, f"../../{scene_info_path}/{task_name}/{setting}/scene_info_.json")
    try:
        for file_path in file_paths:
            with open(file_path, "r") as f:
                scene_data = json.load(f)
            data_all.update(scene_data)
        # ****************modify end xu **************
        return data_all
...
def save_episode_descriptions(task_name: str, setting: str, generated_descriptions: List[Dict]):
    ...
    output_dir = os.path.join(parent_directory, f"../../data_randomized_only_loca/{task_name}/{setting}/instructions")
...
def generate_episode_descriptions(task_name: str, episodes: List[Dict[str, str]], max_descriptions: int = 1000000):
    ...
    for i, episode_data in enumerate(episodes):
        ...
        if "episode_" in episode_idx:
            episode_idx = episode_idx.split("episode_")[1]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--setting",
        type=str,
        default="demo_randomized_only_loca",
    ...
    # 12个task分3个进程，每个进程跑4个task
    for task_name in task_names[args.idx:args.idx+4]:
