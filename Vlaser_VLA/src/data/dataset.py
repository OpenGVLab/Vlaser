...
def make_dataset_from_rlds(
    ...
    *,
    ...
    # def restructure_proprio(traj):
    #     if proprio_obs_key is not None and traj['observation']['prorpio'].shape:
    ...
    if isinstance(dataset_statistics, str) and name != 'fractal20220817_data':
        with tf.io.gfile.GFile(dataset_statistics, "r") as f:
            dataset_statistics = json.load(f)
    elif dataset_statistics is None or name == 'fractal20220817_data':
        ...
        dataset_statistics = get_dataset_statistics(
            ...
            save_dir=None if name == 'fractal20220817_data' and  dataset_mixture   else   builder.data_dir,
            # force_recompute=True if name == 'fractal20220817_data' else False,
    ...
    # dataset = dataset.traj_map(restructure_proprio, num_parallel_calls).filter(
    #     is_nonzero_length
    # )
...
def make_interleaved_dataset(
    ...
    *,
    ...
    for dataset_kwargs in dataset_kwargs_list:
        _, dataset_statistics, _ = make_dataset_from_rlds(**dataset_kwargs, train=train, dataset_mixture = True if len(dataset_kwargs_list)>1 else False)
    ...
    for dataset_kwargs, threads, reads in zip(
        ...
        dataset, _, dataset_len = make_dataset_from_rlds(
            **dataset_kwargs,
            ...
            dataset_mixture = True if len(dataset_kwargs_list)>1 else False,
        )
