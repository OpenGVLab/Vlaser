...
from src.model.vla.processing import InternVLAProcessor, VLAProcessor
...
class TrainAgent:
    def __init__(self, cfg):
        ...
        if 'INTERNVL' in os.environ:
            from src.model.vla.pizero_internvl import PiZero
        else:           
        ...
        if cfg.lora:
            ...
            """All params in action expert are trainable"""
            for name, param in self.model.joint_model.mixtures["action"].named_parameters():
                param.requires_grad = True
        
        if 'visionaction' in cfg:
            for p in self.model.internvl_model.language_model.parameters():
                p.requires_grad = False
            
            for p in self.model.joint_model.mixtures['vlm'].parameters():
                p.requires_grad = False
        ...
        print(f"Trainable params in VLA model: {n_trainable_params / 1024 / 1024}M")
        
        trainable_params = []
        for name, param in self.model.named_parameters():
            if param.requires_grad:
                trainable_params.append(name)
        json_path = os.path.join(self.log_dir, "trainable_params.json")
        
        with open(json_path, "w") as f:
            json.dump(trainable_params, f, indent=4)        
        ...
        trainable_parameters =  model.action_expert_parameters_only if  'actiononly' in cfg else model.trainable_vision_parameters + model.action_expert_parameters  if 'visionaction' in cfg else model.action_expert_parameters
        
        if "debug_param" in os.environ:
            trainable_parameters = model.action_expert_parameters_debug
        ...
        if self.train_vlm:
            if cfg.lora:
                ...
                if 'TRAIN_VISION_ONLY' in os.environ:
                    vlm_trained_parameters = model.trainable_vision_parameters
        ...
        if 'INTERNVL' in os.environ:
            self.processor = InternVLAProcessor(
                self.tokenizer,
                num_image_tokens=cfg.cond_steps*64 if not 'IMAGE_448' in os.environ else cfg.cond_steps*256,
                max_seq_len=cfg.max_seq_len,
                tokenizer_padding=cfg.tokenizer_padding,
                num_images = cfg.cond_steps
            )
        else:
            self.processor = VLAProcessor(
                self.tokenizer,
                num_image_tokens=cfg.vision.config.num_image_tokens * cfg.cond_steps,
                max_seq_len=cfg.max_seq_len,
                tokenizer_padding=cfg.tokenizer_padding,
            )
    ...
    def run(self):
        ...
        loss_deque_vlm = deque(maxlen=self.grad_accumulation_steps)
        ...
        def preprocess_batch(batch, split_mask: bool, sample_fm_time: bool):
            ...
            proprios = batch["observation"]["proprio"][:,-1][:,None,:]
            
            if self.cfg.cond_steps != 1:
                
                actions = batch["action"] # (torch.Size([16, 2, 4, 7])
                actions = torch.cat([actions[:,None, :-1, 0], actions[:, -1:, :]], axis = 2).squeeze(1)
            else:
            ...
            if 'DEBUG_POS_ID' in os.environ:
                vlm_position_ids = vlm_position_ids - 1
        ...
        if int(os.environ["LOCAL_RANK"]) == 0:
            ...
            for name, param in model.named_parameters():
                if not param.requires_grad:
                    log.info(f"{name} requires_grad=False")
                else:
                    log.info(f"{name} requires_grad=True")
            log.info('parameters------------------')
        
        while 1:
            if 'VLM_ONLY' in os.environ:
                self.train_dataloader = range(1000000000000000)
            for batch in self.train_dataloader:
                ...
                if self.run_eval and (cnt_batch + 1) % self.eval_freq == 0:
                    ...
                    with torch.inference_mode():
                        for _ in range(self.per_device_num_eval_batch):
                            ...
                            gt_actions = gt_actions[:, :preds.shape[1]]
                ...
                if cnt_batch % self.log_freq == 0:
                    ...
                    loss_vlm_metric = np.mean(loss_deque_vlm)
                    
                    peak_vram = torch.cuda.max_memory_reserved(self.gpu_id) / (1024**3)
                    log_msg = f"Batch {cnt_batch} Update {cnt_update}: t {timer():8.4f} | vram {peak_vram:6.3f} | loss {loss_metric:6.4f} | loss_vlm {loss_vlm_metric:6.4f} | action lr {self.action_optimizer.param_groups[0]['lr']:10.8f}"
                    if self.train_vlm:
