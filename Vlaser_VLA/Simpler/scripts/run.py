...
def _main(cfg: OmegaConf):
    ...
    agent = cls(cfg)
    if "EVAL_VAR" not in os.environ and "EVAL_MATCH" not in os.environ:
        agent.run()
    else:
        agent.run_variant_aggregation()
...
def main(cfg: OmegaConf):
    if 'TRAIN_VISION_ONLY' not in os.environ:
        if "env" in cfg:
            OmegaConf.set_struct(cfg, False)  # 允许动态添加新键
            cfg.env.adapter.proprio_dim = cfg.proprio_dim
            OmegaConf.set_struct(cfg, True)   # 恢复 Struct 模式（可选）
    _main(cfg)
