...
def get_internvl3(pretrained_model_path='OpenGVLab/InternVL3-2B'):
    print(pretrained_model_path)
    import sys
    
    

    from internvl.model.internvl_chat import InternVLAModel, InternVLChatModel
    ...
    if 'IMAGE_448' not in os.environ:
        vlm.vision_model.resize_pos_embeddings(old_size=448,
                                                    new_size=224,
                                                    patch_size=patch_size)
    
    token_embedding_size = config.llm_config.hidden_size
...
class PiZero(nn.Module, NoSyncBase):
    ...
    def __init__(self, cfg, use_ddp: bool = False):
        ...
        # Change with InternVL3
        internvl_model = get_internvl3(cfg.pretrained_model_path)
        # internvl_model.proprio_expert.model.lm_head = None
        self.multi_modal_projector = internvl_model.mlp1
        self.channel_expand = torch.nn.Sequential(
    ...
    def _forward_siglip_and_text_embedding(
        ...
        if 'IMAGE_448' in os.environ:
            img_feats = img_feats.reshape(img_feats.shape[0], 32, 32, -1)
        else:
            img_feats = img_feats.reshape(img_feats.shape[0], 16, 16, -1)
        # 不做 pixel_shuffle，直接手动 reshape
        # selected_image_feature = img_feats
        selected_image_feature = self.channel_expand(img_feats)  # [B, H, W, 4096]
