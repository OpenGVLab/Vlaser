...
from src.data.utils.droid_utils import droid_baseact_transform
...
def quaternion_to_euler_tf(quaternion):
    ...
    sinp = 2 * (w * y - z * x)
    sinp = tf.clip_by_value(sinp, -1.0, 1.0)  # Avoid NaN if sinp > 1.0
    pitch = tf.asin(sinp)
...
def rt1_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # tf.print(type(trajectory["observation"]["base_pose_tool_reached"]))
    # tf.print(type(tft.euler.from_quaternion(trajectory['observation']['base_pose_tool_reached'][:, 3:7])))
    ...
    # euler_angles = quaternion_to_euler_tf(trajectory['observation']['base_pose_tool_reached'][:, 3:7])
    trajectory['observation']['proprio'] = tf.concat([
...
def rt1_dataset_transform_orig(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    # make gripper action absolute action, +1 = open, 0 = close
    gripper_action = trajectory["action"]["gripper_closedness_action"][:, 0]
    gripper_action = rel2abs_gripper_actions(gripper_action)
...
OXE_STANDARDIZATION_TRANSFORMS = {
    ...
    "fractal20220817_data_orig": rt1_dataset_transform_orig,
