...
from src.agent.variant_aggressive_config import find_suitable_args, find_suitable_args_match,get_env
...
class EvalAgent:
    def __init__(self, cfg):
        ...
        if 'INTERNVL' in os.environ:
            from src.model.vla.pizero_internvl import PiZeroInference
    ...
    def load_checkpoint(self, path):
        ...
        load_result = self.model.load_state_dict(data["model"], strict=False)
        print(load_result)
        assert len(load_result.missing_keys) == 0, 'missing keys'
            
        log.info(f"Loaded model from {path}")


    def run_variant_aggregation(self):
        ...
        # Different tasks with different init
        if "EVAL_VAR" in os.environ:
            config_dict_list = find_suitable_args(self.task)
        else:
             config_dict_list = find_suitable_args_match(self.task)
        
        for config_dict in config_dict_list:
            if config_dict['additional-env-build-kwargs-variants'] == None:
                config_dict['additional-env-build-kwargs-variants'] = [None]
            for env_n in config_dict['env-name']:
                for sce_n in config_dict['scene-name']:
                    for add_arg in config_dict['additional-env-build-kwargs-variants']:
                        for robot_init_x in config_dict["robot-init-xs"]:
                            for robot_init_y in config_dict["robot-init-ys"]:
                                for robot_init_quat in config_dict["robot-init-quats"]:
                                    control_mode = get_robot_control_mode(config_dict["robot"], None)
                                    kwargs = dict(
                                    model = None,
                                    ckpt_path = config_dict['ckpt-path'],
                                    robot_name=config_dict["robot"],
                                    env_name=env_n,
                                    scene_name=sce_n,
                                    ...
                                    additional_env_build_kwargs=add_arg,
                                    rgb_overlay_path=config_dict["rgb-overlay-path"],
                                    control_freq=config_dict["control-freq"],
                                    sim_freq=config_dict["sim-freq"],
                                    max_episode_steps=config_dict["max-episode-steps"],
                                    enable_raytracing=config_dict["enable-raytracing"],
                                    additional_env_save_tags=config_dict["additional-env-save-tags"],
                                    obs_camera_name=config_dict["obs-camera-name"],
                                    logging_dir=config_dict["logging-dir"],
                                )
                                if config_dict["obj-variation-mode"] == "xy":
                                    for obj_init_x in config_dict["obj-init-xs"]:
                                        for obj_init_y in config_dict["obj-init-ys"]:
                                            env, env_reset_options = get_env(
                                ...
                                elif config_dict["obj-variation-mode"] == "episode":
                                    for obj_episode_id in range(config_dict["obj-episode-range"][0], config_dict["obj-episode-range"][1]):
                                        env, env_reset_options = get_env(obj_episode_id=obj_episode_id, **kwargs)
                                        obs, reset_info = env.reset(options=env_reset_options)  
                                        env_adapter.reset()
                                        instruction = env.get_language_instruction()
                                        while 1:
